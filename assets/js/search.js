document.addEventListener('DOMContentLoaded', function () {
    ...
    function isControlPage() {
        // Check if we're on a specific control page (not just a baseline index)
        const path = window.location.pathname;
        return path.match(/\/(high|moderate|low|li-saas)\/[^\/]+\//);
    }
    ...
    // Auto-hide logic: show on index pages, hide on control pages
    function updateMobileMenuVisibility() {
        ...
        if (isControlPage()) {
            ...
            // On index pages, show the menu by default
            showMobileMenu();
        }
    }
    ...
    window.addEventListener('resize', function() {
        if (window.innerWidth >= 768) { // md breakpoint
            // Reset mobile menu state for desktop
            sidebar.classList.remove('-translate-y-full');
            sidebarOverlay.classList.add('hidden');
            document.body.classList.remove('menu-open');
        } else {
            // On mobile, apply auto-hide logic
